...
import com.callor.jc.model.NaverRestLayout;
 ...
 *
 * 인터페이스처럼 구현되지 않은 프로토타입의 method 를 가지면서
 * 일부는 자체적으로 구현된 method 를 포함할 수 있다
 *
 * 인터페이스
 * 클래스를 만들기 위한 설계도
 * method 의 프로토 타입만 존재, 실제 구현되지 않음
 */
@Slf4j
...
public class NaverServiceImplV1 extends NaverService<BookVO> {
    ...
    public List<BookVO> naverList(String queryString) throws URISyntaxException {
        ...
        restList = restTemp.exchange(
                restURI, HttpMethod.GET, httpEntity, NaverRestLayout.class);
